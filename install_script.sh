...
while [[ -z "$git_user_email" ]]; do
    read -r -p "Git email address: " git_user_email
done
git config --global user.name "$git_user_name"
git config --global user.email "$git_user_email"
# Set VSCode as git difftool and mergetool (show diffs with git difftool <file> / git mergetool)
...
################################################################################
sudo apt install python3 python3-pip python3-venv fontconfig libcairo2 procps -y
git clone https://github.com/David0tt/workspace-icon-daemon \
    ~/.linux_autosetup/program_installation/workspace-icon-daemon
python3 -m venv ~/.local/share/workspace-icon-daemon/venv
~/.local/share/workspace-icon-daemon/venv/bin/python -m pip install \
    ~/.linux_autosetup/program_installation/workspace-icon-daemon
